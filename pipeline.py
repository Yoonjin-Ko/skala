...
#    v1.2 (2026-07-21) main() 함수 추가 및 예외처리 강화
================================================================================
"""
...
def main():
    # API 실행
    results = asyncio.run(collect_apis())

    weather_json = None if isinstance(results[0], Exception) else results[0]
    country_json = None if isinstance(results[1], Exception) else results[1]
    ip_json = None if isinstance(results[2], Exception) else results[2]

    weather_data = parse_weather(weather_json)
    country_data = parse_country(country_json)
    ip_data = parse_ip(ip_json)

    # 검증 실행
    weather_result = validate_schema(WeatherSchema, weather_data)
    country_result = validate_schema(CountrySchema, country_data)
    ip_result = validate_schema(IPSchema, ip_data)
    ...
    try:
        ...
        for n_rows in [100, 1000, 10000, 50000]:
            df_sample = df_test.sample(n=n_rows, random_state=42)
            
            start = time.perf_counter()
            df_sample.to_csv(f"tmp_{n_rows}.csv", index=False)
            csv_write = time.perf_counter() - start
            
            start = time.perf_counter()
            df_sample.to_parquet(f"tmp_{n_rows}.parquet")
