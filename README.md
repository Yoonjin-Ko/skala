...
- API 데이터는 항상 신뢰할 수 없기 때문에 검증 단계가 필요하다
- Pydantic을 활용하면 데이터 계약(Data Contract)을 코드로 관리할 수 있다
- 비동기 처리는 여러 외부 I/O 작업에서 효율적인 방법이다
- 데이터가 커질수록 Parquet의 장점이 두드러진다
- CSV는 단순하고 보편적이지만, 대규모에서는 비효율적이다
- 상황에 맞게 CSV와 Parquet을 선택해야 한다

---

## 개선 예정

- [ ] API 실패 시 Retry 처리 추가
- [ ] Logging 적용
- [ ] 환경변수(.env) 기반 API 관리
- [ ] 모듈 구조 분리
- [ ] Docker 실행 환경 추가